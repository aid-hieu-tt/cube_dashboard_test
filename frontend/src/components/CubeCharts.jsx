...
export default function CubeCharts() {
  ...
  // Click handler — dùng trực tiếp trên <Bar> và <Pie>, không đặt trên wrapper
  ...
  // Helper: fill color dựa theo selection
  const getCellFill = (name, index) => {
    const baseColor = COLORS[index % COLORS.length];
    if (!selectedProduct) return baseColor;
    return name === selectedProduct
      ? baseColor
      : `${baseColor}${Math.round(DIM_OPACITY * 255).toString(16).padStart(2, '0')}`;
  };
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 5 }}>
              ...
              <Bar dataKey="quantity" name="Số lượng" radius={[6, 6, 0, 0]} barSize={40}
                   onClick={handleBarItemClick} style={{ cursor: 'pointer' }}>
                {chartData.map((entry, index) => (
                  <Cell key={`cell-q-${index}`} fill={getCellFill(entry.name, index)}
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 5 }}>
              ...
              <Bar dataKey="revenue" name="Doanh thu" radius={[6, 6, 0, 0]} barSize={40}
                   onClick={handleBarItemClick} style={{ cursor: 'pointer' }}>
                {chartData.map((entry, index) => (
                  <Cell key={`cell-r-${index}`} fill={getCellFill(entry.name, index)}
                        stroke={getCellStroke(entry.name)}
                        strokeWidth={getCellStroke(entry.name) !== 'none' ? 2 : 0}
                        style={{ cursor: 'pointer', transition: 'all 0.3s ease' }} />
                ))}
          ...
          <ResponsiveContainer width="100%" height={340}>
            <PieChart>
              <Pie data={pieData} cx="50%" cy="50%" innerRadius={70} outerRadius={120}
                   paddingAngle={3} dataKey="value" nameKey="name" label={renderCustomLabel}
                   animationBegin={0} animationDuration={800}
                   stroke="rgba(15, 23, 42, 0.8)" strokeWidth={2}
                   onClick={handlePieSliceClick}
                   style={{ cursor: 'pointer' }}>
                {pieData.map((entry, index) => (
                  <Cell key={`pie-cell-${index}`} fill={getCellFill(entry.name, index)}
                        stroke={getCellStroke(entry.name)}
